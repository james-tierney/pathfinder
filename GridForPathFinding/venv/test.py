...
"""
    ...
    print( str(grid))
    print("grid val @  " + str(grid[2][1])) #grid values arent lined up properly grid [2][1] when printing from the grid = 1 but on screen = 8
        ...
        if event.type == pygame.QUIT:  # If user clicked close
            ...
            if first_click == 0:
                ...
                if first_click == -1:  # SEEMS TO REQUIRE 2 CLICK FOR DEST BEFORE CALLING SHORTSEST_PATH FUNCTION??
                    ...
                    print("start pos = " + str(start))
                    print("dest pos = " + str(dest))
                    print("val at dest pos = %d" % grid[row][column] )
